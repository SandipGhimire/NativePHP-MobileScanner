...
const VALID_FORMATS = ['qr', 'ean13', 'ean8', 'code128', 'code39', 'upca', 'upce', 'all'];
...
async function bridgeCall(method, params = {}) {
    ...
    if (result.status === 'error') {
        const error = new Error(result.message || 'The scanner could not be started.');
...
function installListener() {
    ...
    document.addEventListener('native-event', (e) => {
        ...
        (_eventListeners[eventName] || []).forEach((callback) => callback(payload, eventName));
    });
